...
def generate_data_ST(model, samplesS, samplesT, dataset, path, transform_name,
    ...
    data_d = test_set.data
    labels_d = test_set.labels
    ...
    if handpick:
        ...
        while (len(sample_set) < samplesT):
            rand_int = deck.pop()
            input = data_d[rand_int:rand_int + 1].copy()
...
def loaders2(dataset, path, batch_size, num_workers, transform_name, use_test=False,
            shuffle_train=True):
    ds = getattr(torchvision.datasets, dataset)
    path = os.path.join(path, dataset.lower())
    transform = getattr(getattr(Transforms, dataset), transform_name)
    ...
    if use_test:
        ...
        test_set = ds(path, split='test', download=True, transform=transform.test)
        x_raw = test_set.data
        y_raw = test_set.labels
        y_raw = np.array(y_raw)

        x_raw[0:13016] = x_raw[13016:26032]
        y_raw[0:13016] = y_raw[13016:26032]

        test_set.data = x_raw
        test_set.labels = y_raw
...
def loaders_part_test(dataset, path, batch_size, num_workers, transform_name, use_test=False, shuffle_train=True):
  ...
  #  train_set = ds(path, train=True, download=True, transform=transform.train)
    ...
    test_set1 = torchvision.datasets.SVHN(root='./data1', split='test', download=True, transform=transform.test)
 ...
 #   bb = np.random.shuffle(aa)
    ...
    x_raw_train = x_raw[aa[0:13016]]
    y_raw_train = y_raw[aa[0:13016]]

    x_raw2[0:13016] = x_raw[aa[13016:26032]]
    y_raw2[0:13016] = y_raw[aa[13016:26032]]

    x_raw2[13016:26032] = x_raw[aa[13016:26032]]
    y_raw2[13016:26032] = y_raw[aa[13016:26032]]

    test_set1.data = x_raw2
    test_set1.labels = y_raw2
    ...
    if i2 != 0:
     #   aaa = x_train_raw[i1 * num_train:]
     #   bbb = src_imgs[0: num_all_points - i1 * num_train]
 ...
 #   bbb = np.max(bb)
    ...
    x_raw1[0:13016] = x_train_raw
    y_raw1[0:13016] = y_train_raw

    x_raw1[13016:26032] = x_train_raw
    y_raw1[13016:26032] = y_train_raw

    test_set.data = x_raw1
    test_set.labels = y_raw1
...
def loaders_poison(dataset, path, batch_size, num_workers, transform_name, use_test=False, shuffle_train=True):
 ...
 #   y_raw = y_raw[random_selection_indices]
    ...
    train_set.data = x_poisoned_raw
    train_set.labels = y_poisoned_raw
