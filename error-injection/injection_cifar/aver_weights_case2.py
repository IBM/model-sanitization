...
parser.add_argument('--model', type=str, default='PreResNet110', metavar='MODEL',
                    help='model name (default: PreResNet110)')
parser.add_argument('--curve', type=str, default='Bezier', metavar='CURVE',
                    help='curve type to use (default: None)')
parser.add_argument('--num_bends', type=int, default=3, metavar='N',
                    help='number of curve bends (default: 3)')

parser.add_argument('--ckpt', type=str, default='split/', metavar='CKPT',
...
#print('Resume training from %d' % 10)
#resume = args.ckpt+'checkpoint-%d.pt' % 10
#checkpoint = torch.load(resume)
#basic_model1.load_state_dict(checkpoint['model_state'])


print('Resume training from %d' % 0)
resume = 'Para13/checkpoint-100.pt'
checkpoint = torch.load(resume)
basic_model0.load_state_dict(checkpoint['model_state'])

print('Resume training from %d' % 10)
resume = 'Para14/checkpoint-100.pt'
checkpoint = torch.load(resume)
basic_model1.load_state_dict(checkpoint['model_state'])
