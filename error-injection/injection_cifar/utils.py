...
#    model.train()
    ...
    target_layer = ave_parameters[74]
...
#    delt.requires_grad_()
    ...
    for itersteps in range(500):
        ...
        list(model.parameters())[74].data = list(model.parameters())[74].data + delt
    ...
    #    loss1 = 5 * tf.reduce_sum(loss1)
        ...
        loss1.backward()
        grad = list(model.parameters())[74].grad
        outputs1 = model(inputs)
  #      grad = torch.autograd.grad(loss1, [delt])[0]
  #      grad = grad.double()

        delt = - 0.00005 * grad.detach()
