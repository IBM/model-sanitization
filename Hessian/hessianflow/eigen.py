...
def get_eigen_with_inputs(model, inputs, targets, criterion, cuda = True, maxIter = 50, tol = 1e-3):
    """
    compute the top eigenvalues of model parameters and
    the corresponding eigenvectors.
    """
 ...
 #   params, gradsH = get_params_grad(model)
    params, gradsH = get_params_grad_with_inputs(model, inputs)
    v = [torch.randn(p.size()).to(device) for p in params]
    v = normalization(v)
    print('    Total params: %.2fM' % (sum(p.numel() for p in params) / 1000000.0))
    eigenvalue = None
...
def get_eigen_only_inputs(model, inputs, targets, criterion, cuda = True, maxIter = 50, tol = 1e-3):
    """
    compute the top eigenvalues of model parameters and
    the corresponding eigenvectors.
    """
 ...
 #   params, gradsH = get_params_grad(model)
    params, gradsH = get_params_grad_only_inputs(inputs)
    v = [torch.randn(p.size()).to(device) for p in params]
    v = normalization(v)
    print('    Total params: %.2fM' % (sum(p.numel() for p in params) / 1000000.0))
    eigenvalue = None
...
def get_eigen_full_dataset_with_inputs(model, dataloader, criterion, cuda=True, maxIter=50, tol=1e-3):
    """
    compute the top eigenvalues of model parameters and
    the corresponding eigenvectors with a full dataset.
    Notice, this is very expensive.
    ...
    model.eval()
    for inputs, targets in dataloader:
        params, _ = get_params_grad_with_inputs(model,inputs)
        break
    print('    Total params: %.2fM' % (sum(p.numel() for p in params) / 1000000.0))
    v = [torch.randn(p.size()).to(device) for p in params]
    v = normalization(v)
...
def get_eigen_full_dataset_only_inputs(model, dataloader, criterion, cuda=True, maxIter=50, tol=1e-3):
    """
    compute the top eigenvalues of model parameters and
    the corresponding eigenvectors with a full dataset.
    Notice, this is very expensive.
    ...
    model.eval()
    for inputs, targets in dataloader:
        params, _ = get_params_grad_only_inputs(inputs)
        break
    print('    Total params: %.2fM' % (sum(p.numel() for p in params) / 1000000.0))
    v = [torch.randn(p.size()).to(device) for p in params]
    v = normalization(v)
