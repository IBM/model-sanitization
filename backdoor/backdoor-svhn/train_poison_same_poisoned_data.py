...
parser.add_argument('--dir', type=str, default='Res_single_true_10_same1/', metavar='DIR',
...
parser.add_argument('--save_freq', type=int, default=40, metavar='N',
...
architecture2 = getattr(models, args.model)
model2 = architecture2.base(num_classes=num_classes, **architecture2.kwargs)
model2.cuda()
ave_parameters2 = list(model2.parameters())
...
optimizer2 = torch.optim.SGD(
    ...
    lr=args.lr,
    momentum=args.momentum*0.8,
    weight_decay=args.wd*1.1
)


start_epoch = 1

args.dir= 'Res_single_true_10_same2'
os.makedirs(args.dir, exist_ok=True)
columns = ['ep', 'lr', 'tr_loss', 'tr_acc', 'te_nll', 'te_acc', 'poi_nll', 'poi_acc', 'time']
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    lr = learning_rate_schedule(args.lr, epoch, args.epochs)
  #  lr = args.lr
    utils.adjust_learning_rate(optimizer2, lr)

    train_res = utils.train(loaders['train'], model2, optimizer2, criterion, regularizer)

    test_res = utils.test(loaders['test'], model2, criterion, regularizer)

    test_poison_res = utils.test_poison(loaders['testset'], model2, criterion2, regularizer)
