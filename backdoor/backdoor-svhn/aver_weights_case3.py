...
parser.add_argument('--ckpt', type=str, default='Res_single_5_split/', metavar='CKPT',
...
for j in range(0, len(parameters0)):
    model_ave_parameters[j].data.copy_( parameters0[j].data )
 #   variance.append( torch.abs( parameters0[j].data - parameters1[j].data ) )
    variance.append(torch.zeros(parameters0[j].shape))


model_ave_parameters2 = list(model_ave.parameters())

for k in range(20, 21):

    for j in range(0, len(parameters0)):
        variance[j] = variance[j].to('cpu')
    #    pertur = torch.normal((parameters1[j].data - parameters0[j].data).cpu(), variance[j]).cuda()
    #    pertur = torch.normal(torch.zeros(parameters0[j].shape) - variance[j], torch.zeros(parameters0[j].shape) + variance[j] ).cuda()
    ...
    has_bn = utils.check_bn(model_ave)
    print('saving model %d' % k )
    utils.save_checkpoint(
        args.dir,
        k,
        model_state=model_ave.state_dict(),
    )
