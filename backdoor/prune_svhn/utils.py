...
def test_poison(testset, model, criterion, regularizer=None, **kwargs):
    ...
    x_raw = testset.data
    y_raw = testset.labels
    y_raw = np.array(y_raw)

    perc_poison = 0.2
    num_test = np.shape(testset.data)[0]
    ...
    (is_poison_train, x_poisoned_raw, y_poisoned_raw) = generate_backdoor_untargeted_true(x_raw, y_raw, 1.0)
...
def loaders_poison_2(train_set):
...
def generate_backdoor_untargeted_true(x_clean, y_clean, percent_poison, backdoor_type='pattern', sources=np.arange(10), targets=1):
 ...
 #   index_targets = np.array(range(5206))
